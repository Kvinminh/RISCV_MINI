...
interface if_stage_if(input logic clk);
    import core_pkg::*;
...
class if_stage_transaction;
    ...
    // Keep normal fetch common,
    // but force redirect cases to appear.
    // -------------------------
    constraint c_control_weight {
        stall_pc_i dist {0 := 75, 1 := 25};
        jal        dist {0 := 85, 1 := 15};
        jalr       dist {0 := 85, 1 := 15};
        br_en      dist {0 := 75, 1 := 25};
        br_taken   dist {0 := 70, 1 := 30};
    }
    ...
    function string convert2string();
        return $sformatf(
            "stall=%0b br_en=%0b br_taken=%0b jal=%0b jalr=%0b "
...
// ============================================================================
// 4. REFERENCE MODEL CLASS
// ============================================================================
// Purpose:
//   Independently calculate the expected next PC.
//
// DUT rule from current pc.sv + pc_mux.sv:
//
//   if reset       -> PC = 0
//   else if stall  -> PC keeps old value
//   else if jump   -> PC = jump_addr
//   else           -> PC = old_PC + 4
//
// jump condition:
//   (br_taken && br_en) || jal || jalr
// ============================================================================
...
class if_stage_ref_model;
    ...
    function bit [31:0] predict(
        ...
        else if ((br_taken && br_en) || jal || jalr) begin
            model_pc = jump_addr;
        end
        else begin
...
// ============================================================================
// 7. FUNCTIONAL COVERAGE CLASS
// ============================================================================
// Purpose:
//   Measure whether IF-stage scenarios were actually exercised.
//
// COVERAGE MODEL:
//
//   - Stall coverage
...
// Coverage measures scenario exercise.
// ============================================================================
...
class if_stage_test;
    ...
    task run_oop_test();
        ...
        // Allow monitor/scoreboard/coverage to consume final transaction.
