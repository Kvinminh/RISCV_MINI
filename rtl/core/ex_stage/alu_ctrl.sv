...
(
    ...
    always_comb begin
        ...
        unique case (alu_op)
            ...
            ALU_RTYPE: begin
                unique case (f3)
                    F3_ADD_SUB: alu_ctrl_o = alu_ctrl_e'(f7_5 ? ALU_SUB : ALU_ADD);
                    F3_SLL:     alu_ctrl_o = ALU_SLL;
                    F3_SLT:     alu_ctrl_o = ALU_SLT;
                    F3_SLTU:    alu_ctrl_o = ALU_SLTU;
                    F3_XOR:     alu_ctrl_o = ALU_XOR;
                    F3_SRL_SRA: alu_ctrl_o = alu_ctrl_e'(f7_5 ? ALU_SRA : ALU_SRL);
            ...
            ALU_ITYPE: begin
                unique case (f3)
                    ...
                    F3_SRLI_SRAI: alu_ctrl_o = alu_ctrl_e'(f7_5 ? ALU_SRA : ALU_SRL);
